...
describe('Test Teacher route api', () => {
    ...
    it('Register teacher and students with invalid email', (done: CallbackHandler) => {
        ...
        return request(app.use(routes))
            ...
            .expect(400)
            .expect((res) => {
                expect(res.body && res.body.details && res.body.details[0] && res.body.details[0].message && res.body.details[0].message.includes('must be a valid email') || false).toBeTruthy();
                expect(spyTServiceRegister).toHaveBeenCalledTimes(0);
                spyTServiceRegister.mockClear();
            })
            .end(done);
    ...
    it('Get common Students with one teacher fail with unregistered teacher email', (done: CallbackHandler) => {
        ...
        return request(app.use(routes))
            ...
            .expect((res) => {
                expect(res.body && res.body.error && res.body.error.message && res.body.error.message.includes('Teacher does not exist.') || false).toBeTruthy();
    ...
    it('Get common Students with two teacher with one invalid teacher email', (done: CallbackHandler) => {
        ...
        return request(app.use(routes))
            .get('/api/commonstudents')
            .query({
                teacher: [teacherJson[0].email.replace('@', ''), teacherJson[1].email.replace('@', '')],
            })
            .expect(400)
            .expect((res) => {
                expect(res.body && res.body.details && res.body.details[0] && res.body.details[0].message && res.body.details[0].message.includes('must be a string') || false).toBeTruthy();
                expect(res.body && res.body.details && res.body.details[1] && res.body.details[1].message && res.body.details[1].message.includes('must be a valid email') || false).toBeTruthy();
    ...
    it('Suspend student', (done: CallbackHandler) => {
        ...
        return request(app.use(routes))
            ...
            .expect(204)
            .expect(async() => {
                const student = await connection.getRepository(Student).findOne({email: studentEmail});
    ...
    it('Suspend student fail with unregistered email', (done: CallbackHandler) => {
        ...
        return request(app.use(routes))
            ...
            .expect(400)
            .expect((res) => {
                expect(res.body && res.body.error && res.body.error.message && res.body.error.message.includes('Student does not exist.') || false).toBeTruthy();
                // expect(spyTServiceSuspendStudent).toHaveBeenCalledTimes(1);
                // spyTServiceSuspendStudent.mockClear();
            })
            .end(done);
    ...
    it('Retrieve notification with suspended student and unregistered student', (done: CallbackHandler) => {
        ...
        return request(app.use(routes))
            ...
            .expect(200)
            .expect((res) => {
                expect(res.body.data.recipients.every((i:string) => students.includes(i))).toBeTruthy();
